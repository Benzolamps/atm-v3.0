...
public class DepositView extends AbstractView {
    ...
    private void inputAmount(ViewOperation operation, ViewTarget response) {
        ...
        // 获取用户输入的金额
        String amountStr = regexValidation(msg("CI"), rgx("amount"), msg("UE1")).execute();
        
        amount = Double.valueOf(amountStr);
        
        // 打印金额
        Console.write(msg("CC"), amount);
    ...
    private void confirmAmount(ViewOperation operation, ViewTarget response) { 
        ...
        Integer result = dao.deposit(user, amount);
        
        user = dao.getUserByCardId(user.getCardId());
        Double balance = user.getBalance();
        
        // 打印存款结果
        Console.write(msg("CR"), result == 1 ? msg("CS") : msg("UE3"));
        
        // 打印存款金额
        Console.write(msg("CB"), amount, balance);
